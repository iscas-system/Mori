...
public:
    EventBasedMemoryScheduler(const Context& _context, status::MemoryStatus& _status, events::Events& _events): MemoryScheduler(_context, _status, _events) {}
    ...
    virtual std::unordered_set<std::string> analyzeForwardEvents(const events::EventSet<events::MemoryEvent>& iter_1_forward_mem_res) override {
        ...
        for (auto &s : status.getExecutionOrder()) {
            ...
            for (auto &tensor_name : op_pres.getTensors()) { 
                ...
                if (tensor_pres.isPersistent()) continue;
...
public:
    FIFOMemoryScheduler(const Context& _context, status::MemoryStatus& _status, events::Events& _events): EventBasedMemoryScheduler(_context, _status, _events) {}
...
public:
    ExecutionTimeAwareMemoryScheduler(const Context& _context, status::MemoryStatus& _status, events::Events& _events): FIFOMemoryScheduler(_context, _status, _events) { time_model.setStrongSynchronization(false); }
...
public:
    SectionAwareMemoryScheduler(const Context& _context, status::MemoryStatus& _status, events::Events& _events): ExecutionTimeAwareMemoryScheduler(_context, _status, _events) { time_model.setStrongSynchronization(true); }
    virtual ~SectionAwareMemoryScheduler() = default;
};  // struct SectionAwareMemoryScheduler

struct DependencyAwareMemoryScheduler : public FIFOMemoryScheduler {
private:
    struct TensorRelation {
        std::string original_operator   = "";
        std::string current_operator    = "";
        bool schedule_changed           = false;

        TensorRelation(const std::string op): original_operator(op), current_operator(op), schedule_changed(false) {}
...
public:
    DependencyAwareMemoryScheduler(const Context& _context, status::MemoryStatus& _status, events::Events& _events): FIFOMemoryScheduler(_context, _status, _events) {}
    ...
    virtual void onMemoryEvent(const events::MemoryEvent& event) override {
        ...
        if (event.stage == ApplicationStage::backward && (event.type == events::MemoryEventType::swapin || event.type == events::MemoryEventType::swapout)) {
            ...
            if (p->second.current_operator == p->second.original_operator && p->second.schedule_changed) return;

            std::string opb = "";
            if (event.type == events::MemoryEventType::swapin) opb = status.getExecutionPrev(p->second.current_operator);
            else opb = status.getExecutionPost(p->second.current_operator);
